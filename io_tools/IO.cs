...
using Godot;
using MonsterVial.Results;
using MonsterVial.Extensions;
...
namespace MonsterVial
{
  ...
  /// Use realative_path if you want it relative to the script you are in.
  public static class IO
  {
    #region ____ Path Operations ____
    ...
    private static void __LocalizeIfRelative( ref string path, string callerPath = "" )
    {
    ...
    private static string __RemovePrefix( string s, string prefix ) => prefix.Length == 0 ? s : s.Remove(0, prefix.Length);

    #endregion ==== Path Operations ====



    #region ____ Directory and File Operations ____
    ...
    {
      __LocalizeIfRelative( ref dirpath, callerPath );
    ...
    {
      __LocalizeIfRelative( ref path, callerPath );
    ...
    {
      __LocalizeIfRelative( ref filepath, callerPath );
      __LocalizeIfRelative( ref destFilepath, callerPath );
    ...
    {
      __LocalizeIfRelative( ref dirpath, callerPath );
      __LocalizeIfRelative( ref destDirpath, callerPath );
      ...
      foreach (var item in contents)
      {
        ...
        var itemDest = destDirpath.PlusFile( __RemovePrefix( item, dirpath ) );
    ...
    #region ____ Resource Loading/Saving ____
      ...
      where TResource : Resource
    {
      ...
      if (res != null)
      {
        if (IOToolsPlugin.CALL_READY_ON_RESOURCES)
        {
          ...
          if (readyMethod != null)
          {
            try { readyMethod.Invoke( res, __emptyObj ); }
            catch (System.Exception e) { GD.PushError( $"Exception while calling '_Ready' on res '{path}':  {e}" ); }
          }
        }
        return res;
      }
