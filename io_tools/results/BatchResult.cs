...
namespace MonsterVial.Results
{
  ...
  public class BatchResult : Result
  {
    ...
    public override void SetError( Error error, string p_error_message = "" )
    {
      base.SetError( error, p_error_message );
      if (IsOk && errors.Count > 0) { SetError( Error.Failed ); }
    }

    // Ensures failure if errors were added.
    public override void SetError( Result other, string p_error_message = "" )
    {
      base.SetError( other, p_error_message );
      if (IsOk && errors.Count > 0) { SetError( Error.Failed ); }
    }
  };
}
